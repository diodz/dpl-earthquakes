...
First, we now make the identification boundary explicit at the start of Section~5 and in the corresponding footnote: our SCM design identifies the aggregate divergence in outcomes, but does not causally isolate any single institutional channel (including insurance liquidity) without micro-level variation or more granular data.
...
\paragraph{Interpretation}
For Canterbury, NTL confirms the GDP-based result: positive post-treatment divergence (about +10.0\% in 2016; average post-gap about +7.8\%). For Maule, NTL diverges from the GDP-null result (positive NTL post-gap). We now state this explicitly in the manuscript (see details in Appendix B) and discuss it as evidence that remote-sensing proxies can capture broader spatial re-lighting and infrastructure restoration dynamics that do not map one-to-one to measured value-added in regional GDP accounts. These NTL measurement problems are especially acute in regions that have economic activities that rely heavily on agricultural and rural activities (such as Maule), which could suggest that part of that output does not emit nightlight proportional to its economic value. Second, it is known that the DMSP-OLS sensor suffers from measurement limitations, including top-coding, pixel blooming, inter-annual calibration noise, and atmospheric sensitivity (Gibson et al., 2021), all of which are problematic for level comparisons. 
