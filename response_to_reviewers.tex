...
We added decomposition figures and text for both regions (Section~4, Results). For Canterbury, Figure~\ref{fig:nz_decomposition_paths} shows the treated vs.\ synthetic paths for total GDP and population; Figure~\ref{fig:nz_decomposition_gaps} displays the gaps: by 2016, the GDP-per-capita gap is 12.7\%, the total-GDP gap is 5.9\%, and the population gap is $-3.6$\%; averaging over 2011--2022, the gaps are 6.8\% (GDP per capita), 5.0\% (total GDP), and $-3.6$\% (population). For Maule, Figure~\ref{fig:maule_decomposition_paths} and Figure~\ref{fig:maule_decomposition_gaps} show the same three outcomes; gaps remain near zero for GDP per capita, total GDP, and population, so the null result is not an artifact of migration or denominator composition.

\paragraph{Interpretation}
Canterbury's per-capita gap survives the decomposition: total GDP rises above synthetic and population remains below synthetic, so the per-capita effect reflects both a stronger numerator and a smaller denominator. For Maule, the decomposition shows null-like gaps across all three outcomes, confirming that the absence of a per-capita overshoot is not driven by offsetting migration or composition effects.
