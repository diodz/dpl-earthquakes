...
\paragraph{Predictor-weight and predictor-set symmetry sensitivity:} Next we document two additional implementation checks for robustness. First, we vary the SCM predictor-weight optimization settings (initialization and method) while holding donor pools and timing fixed. Second, we re-estimate both country models under a harmonized reduced predictor set that uses common broad sectors (agriculture, manufacturing, construction, trade/hospitality, transport/information, finance/business, personal services, and public administration), while retaining country-specific auxiliary controls (GDP per capita and education). These diagnostics are exported in \texttt{article\_assets/predictor\_spec\_sensitivity.csv} and confirm that the qualitative treatment conclusions are not driven by one specific $v_m$ tuning choice or by unavoidable data asymmetries such as the Chile-specific Fishing category.
...
\includegraphics[height=0.2\textheight]{article_assets/Maule_map.png}\\
\caption{Chile, highlighting Maule Region}
\end{subfigure}
\hfill
\begin{subfigure}{0.42\textwidth}\centering
\includegraphics[height=0.2\textheight]{article_assets/Canterbury_map.png}\\
...
\midrule
New~Zealand & Construction share & +2.53 p.p. & +42.70\% & 19.13 & 0.00 \\
New~Zealand & Non-construction share & $-2.13$ p.p. & $-2.28$\% & 7.79 & 0.00 \\
New~Zealand & Non-construction level & +789.01 & +3.14\% & 1.92 & 0.80 \\
Chile & Construction share & +1.82 p.p. & +25.48\% & 1.50 & 0.17 \\
Chile & Non-construction share & $-2.01$ p.p. & $-2.16$\% & 1.57 & 0.33 \\
Chile & Non-construction level & $-1.62$ & $-6.66$\% & 1.08 & 0.42 \\
\bottomrule
\end{tabular}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
...
We export complete machine-readable outputs to \texttt{article\_assets/timing\_sensitivity\_summary.csv}, \texttt{article\_assets/timing\_sensitivity\_gaps.csv}, and \texttt{article\_assets/timing\_sensitivity\_placebo\_distribution.csv}, with country-specific summaries in \texttt{nz\_timing\_sensitivity\_summary.csv} and \texttt{chile\_timing\_sensitivity\_summary.csv}.
...
We additionally run two further sensitivity checks for the uniform sets. First, for Chile we vary the pre-treatment fit filter used to retain placebo units (2$\times$, 5$\times$, and 10$\times$ the treated pre-RMSPE). The conclusion is unchanged: Maule’s post-treatment uniform set continues to include zero throughout. Second, for New~Zealand we re-estimate the design under alternative treatment-start definitions (2010 vs.\ 2011). In both definitions, Canterbury’s gap remains positive with exclusion of zero concentrated in early-to-mid rebuild years, and re-convergence later in the sample.
...
\hfill
\begin{subfigure}{0.48\textwidth}\centering
\includegraphics[width=\textwidth]{article_assets/nz_uniform_treatment_timing_sensitivity.png}
\caption{New Zealand: treatment-start definitions}
\end{subfigure}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Panel (a) varies Chile's placebo pre-fit filter (2$\times$, 5$\times$, 10$\times$). Panel (b) re-estimates New~Zealand with treatment starting in 2010 and 2011. The qualitative inference is stable across both exercises.
\end{minipage}
\end{figure}
...
\midrule
New~Zealand & Goods (non-construction) & +0.70 & 1.01 & 0.67 \\
New~Zealand & Services (non-construction) & $-3.99$ & 4.35 & 0.07 \\
Chile & Goods (non-construction) & $-4.35$ & 1.64 & 0.42 \\
Chile & Services (non-construction) & +5.62 & 2.67 & 0.17 \\
\bottomrule
\end{tabular}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
