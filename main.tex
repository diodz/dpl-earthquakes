...
\paragraph{In-Time Placebo (Falsification) Test:} We run a \emph{rolling} in-time placebo for every possible pre-treatment year (New Zealand: 2000--2009 and actual 2011; Chile: 1990--2009 and actual 2010). For each candidate year we re-estimate SCM and plot the resulting gap path (Figure~\ref{fig:rolling_in_time_placebo}). The post-treatment divergence is unique to the actual treatment year; placebo years show no sustained divergence, as expected.\\
...
We also perform a \emph{rolling} in-time placebo for every possible pre-treatment year as requested: we re-estimate SCM for each candidate treatment year $T$ in the pre-period and plot the resulting gap path. For New Zealand we use $T \in \{2000,\ldots,2009, 2011\}$ and for Chile $T \in \{1990,\ldots,2009, 2010\}$. Figure~\ref{fig:rolling_in_time_placebo} shows that the post-treatment divergence is unique to the actual treatment year (Canterbury's gap rises only when 2011 is used; Maule remains near zero for 2010 and for all placebo years), so the results are not driven by pre-existing diverging trends.

\begin{figure}[H]\centering
\caption{\textbf{Rolling in-time placebo: gap path for every pre-treatment year}}
\label{fig:rolling_in_time_placebo}
\includegraphics[width=0.95\textwidth]{article_assets/rolling_in_time_placebo_paths.png}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Each gray line is the gap path when treatment is assumed to start in that year; the red line is the path for the actual treatment year (2011 for Canterbury, 2010 for Maule). The divergence is unique to the actual treatment year.
\end{minipage}
\end{figure}
