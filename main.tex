...
\paragraph{In-Time Placebo (Falsification) Test:} We run a \emph{rolling} in-time placebo: for each candidate treatment year we re-estimate SCM and plot the resulting gap path (Figure~\ref{fig:rolling_in_time_placebo}). For New Zealand we use candidate years 2001--2009 and the actual treatment year 2011; for Chile, 1991--2009 and 2010, ensuring at least one pre-treatment year per fit. The post-treatment divergence is unique to the actual treatment year; placebo years show no sustained divergence, as expected.\\
...
We also perform a \emph{rolling} in-time placebo: we re-estimate SCM for each candidate treatment year $T$ in the pre-period and plot the resulting gap path. For New Zealand we use $T \in \{2001,\ldots,2009, 2011\}$ and for Chile $T \in \{1991,\ldots,2009, 2010\}$, with at least one pre-treatment year per fit. Figure~\ref{fig:rolling_in_time_placebo} shows that the post-treatment divergence is unique to the actual treatment year (Canterbury's gap rises only when 2011 is used; Maule remains near zero for 2010 and for all placebo years), so the results are not driven by pre-existing diverging trends.
...
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Each line is the gap path when treatment is assumed to start in a given year; the placebo treatment year is indicated by color (colorbar). The thick red line is the path for the actual treatment year (2011 for Canterbury, 2010 for Maule). Candidate years are 2001--2009 and 2011 (New Zealand) and 1991--2009 and 2010 (Chile), with at least one pre-treatment year per fit. The divergence is unique to the actual treatment year.
\end{minipage}
\end{figure}
