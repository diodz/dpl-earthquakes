...
\paragraph{Uniform Confidence Sets:} Following \textcite{firpo2018synthetic}, we construct uniform confidence bands for the treatment effect that are valid across all post-treatment periods. Unlike pointwise confidence intervals or pseudo-$p$ values from placebo rankings, uniform confidence sets account for the multiple testing problem inherent in examining treatment effects over multiple time periods simultaneously. The approach uses the maximum absolute standardized gap across all post-treatment periods from the placebo distribution to construct confidence bands that maintain the desired coverage probability uniformly over time. Specifically, we standardize each placebo gap by its pre-treatment root mean squared prediction error (RMSPE), compute the maximum absolute standardized gap for each placebo unit over the post-treatment horizon, and use the $(1-\alpha)$ quantile of this distribution as the critical value for constructing uniform $(1-\alpha) \times 100\%$ confidence bands. This provides a rigorous assessment of whether the treatment effect is significantly different from zero at any point in the post-treatment period, controlling for the familywise error rate across time.\\
...
\begin{figure}[H]\centering
\caption{\textbf{Gap in Real GDP per Capita with Uniform 90\% Confidence Bands}}
\label{fig:gaps_uniform_ci}
\begin{subfigure}{0.48\textwidth}\centering
\includegraphics[width=\textwidth]{article_assets/maule_gap_uniform_ci.png}
\caption{Maule (Chile) with Uniform CI}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}\centering
\includegraphics[width=\textwidth]{article_assets/nz_gap_uniform_ci.png}
\caption{Canterbury (NZ) with Uniform CI}
\end{subfigure}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} The treatment gap (red line) is shown with uniform 90\% confidence bands (gray shaded region) constructed following \textcite{firpo2018synthetic}. The uniform confidence sets account for multiple testing across all post-treatment periods. For Maule (panel a), the gap remains within the confidence band, indicating no statistically significant treatment effect. For Canterbury (panel b), the gap exits the confidence band after 2012, confirming a statistically significant positive treatment effect that persists through the post-earthquake period.
\end{minipage}
\end{figure}
