...
In addition to the jackknife test, we implement explicit SUTVA/spillover diagnostics and geographically structured donor exclusions for both cases (Appendix~\ref{sec:spillover_appendix}). For Chile, we progressively exclude O’Higgins (adjacent) and then O’Higgins plus the southern near-ring (Araucanía). For New~Zealand, we progressively exclude the reviewer-prioritized network hubs (Auckland and Wellington), then adjacent South Island donors, and finally both sets together. Table~\ref{tab:spillover_exclusion_main} reports side-by-side estimates.

\begin{table}[H]
\centering
\caption{\textbf{Baseline vs.\ spillover-robust donor pools (GDP per capita SCM)}}
\label{tab:spillover_exclusion_main}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{llcccc}
\toprule
\textbf{Country} & \textbf{Donor-pool rule} & \textbf{Mean post gap (\%)} & \textbf{$\Delta$ vs.\ baseline (p.p.)} & \textbf{Pre RMSPE (\%)} & \textbf{Post/Pre} \\
\midrule
Chile & Baseline donor pool & $-1.71$ & 0.00 & 3.38 & 0.74 \\
Chile & Exclude O’Higgins & $-5.42$ & $-3.71$ & 7.04 & 0.80 \\
Chile & Exclude O’Higgins + Araucanía & $-0.68$ & +1.03 & 4.38 & 0.67 \\
\midrule
New~Zealand & Baseline donor pool & +7.78 & 0.00 & 1.00 & 8.78 \\
New~Zealand & Exclude Auckland + Wellington & +10.47 & +2.68 & 4.40 & 2.52 \\
New~Zealand & Exclude adjacent South Island ring & +9.55 & +1.77 & 2.33 & 4.36 \\
New~Zealand & Exclude ring + network hubs & +12.71 & +4.93 & 5.01 & 2.66 \\
\bottomrule
\end{tabular}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Post windows are 2010--2019 for Chile and 2011--2019 for New~Zealand. Values are from \texttt{article\_assets/spillover\_exclusion\_summary.csv}. The strictest Chile specification remains non-positive ($-0.68\%$), while the strictest New~Zealand specification remains strongly positive (+12.71\%). Full gap paths are reported in Appendix Figure~\ref{fig:spillover_gap_appendix}.
\end{minipage}
\end{table}
...
\section{Spillover diagnostics and geographically structured donor exclusions}
\label{sec:spillover_appendix}

This appendix reports the explicit spillover-channel diagnostics requested by reviewers (migration/population flows, labor-movement proxies, and trade/input-output link proxies) plus progressive exclusion rules for both countries. Reproducible outputs are written to \texttt{article\_assets/spillover\_channel\_diagnostics.csv}, \texttt{spillover\_channel\_group\_summary.csv}, \texttt{spillover\_exclusion\_summary.csv}, and \texttt{spillover\_exclusion\_gaps.csv}.

\begin{table}[H]
\centering
\caption{\textbf{Spillover-channel proxies and data sources}}
\label{tab:spillover_data_sources}
\small
\setlength{\tabcolsep}{6pt}
\begin{tabular}{p{2.8cm} p{4.7cm} p{3.3cm} p{3.3cm}}
\toprule
\textbf{Channel} & \textbf{Operational proxy} & \textbf{Chile source} & \textbf{New~Zealand source} \\
\midrule
Migration / population flows &
Post- minus pre-period annual population-growth differential (regional, p.p.) &
INE regional population series (processed in \texttt{inter/processed\_chile.csv}) &
Stats NZ regional population release \\
\addlinespace
Labor movement &
Chile: tertiary enrollment per-capita shift (labor-supply proxy, p.p.); NZ: construction GDP-share shift (labor-demand proxy, p.p.) &
MINEDUC tertiary enrollment series (processed) &
Stats NZ industry GDP shares (construction) \\
\addlinespace
Trade / input-output linkage proxy &
Pre-treatment correlation in regional GDP-per-capita growth with the treated unit &
Banco Central regional GDP per capita series &
Stats NZ regional GDP per capita series \\
\bottomrule
\end{tabular}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Proxies are used because bilateral migration microflows, inter-regional commuting microdata, and formal regional input-output matrices are not jointly available at comparable frequency for both countries over the full horizon.
\end{minipage}
\end{table}

\begin{table}[H]
\centering
\caption{\textbf{Reviewer-prioritized units: explicit spillover diagnostics}}
\label{tab:spillover_key_units}
\small
\setlength{\tabcolsep}{6pt}
\begin{tabular}{llccc}
\toprule
\textbf{Country} & \textbf{Region / donor group} & \textbf{Pre-growth corr. with treated} & \textbf{Population-growth shift (p.p.)} & \textbf{Labor-proxy shift (p.p.)} \\
\midrule
Chile & O’Higgins (VI) & 0.95 & $-0.04$ & +1.04 \\
New~Zealand & Auckland & 0.78 & +0.26 & +0.27 \\
New~Zealand & Wellington & 0.31 & +0.34 & +0.03 \\
New~Zealand & Adjacent South Island ring (mean) & 0.64 & +0.25 & $-0.15$ \\
\bottomrule
\end{tabular}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Population shifts are post-pre differences in annual growth rates. Chile labor proxy is tertiary enrollment per capita; NZ labor proxy is construction share of regional GDP. Full region-by-region diagnostics appear in \texttt{spillover\_channel\_diagnostics.csv}.
\end{minipage}
\end{table}

\begin{figure}[H]\centering
\caption{\textbf{Spillover-channel diagnostics by donor region}}
\label{fig:spillover_flow_appendix}
\includegraphics[width=0.98\textwidth]{article_assets/spillover_flow_proxies.png}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Top row: migration proxy (population-growth shift). Bottom row: labor proxy shift. Colors identify risk groups used in progressive exclusions (adjacent/ring/network). 
\end{minipage}
\end{figure}

\begin{figure}[H]\centering
\caption{\textbf{Gap paths under progressive spillover exclusions}}
\label{fig:spillover_gap_appendix}
\includegraphics[width=0.98\textwidth]{article_assets/spillover_exclusion_gap_paths.png}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Baseline and exclusion scenarios correspond to Table~\ref{tab:spillover_exclusion_main}. Under the strictest exclusion, Chile moves from $-1.71\%$ to $-0.68\%$ (+1.03 p.p.), while New~Zealand moves from +7.78\% to +12.71\% (+4.93 p.p.).
\end{minipage}
\end{figure}
