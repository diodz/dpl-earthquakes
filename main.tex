...
Given clear cross-case heterogeneity in shock geography (single rural-coastal mega-event versus urban multi-shock sequence), we treat comparability as an empirical design problem rather than a maintained assumption. In addition to country-specific SCM estimates, we therefore report: (i) scale-normalized impact metrics (gap per unit of measured damage), (ii) timing sensitivity around treatment-year definitions in both countries, and (iii) spatially explicit checks that isolate urban-core and Christchurch-centered buffer dynamics from full-region aggregates. These additions do not make the shocks physically identical; instead, they test whether the qualitative divergence (Canterbury positive, Maule near-null) survives when differences in shock geography and measured damage scale are explicitly accounted for.
...
The remainder of the paper is structured as follows: Section 2 reviews the related literature on the economic impact of earthquakes and the role of institutions in disaster recovery. Section 3 describes the data and the SCM methodology employed. Section 4 presents the main results for the Maule and Canterbury cases, including sectoral analysis and a battery of robustness checks. Section 5 discusses the institutional context in each country and ties these factors to the observed outcomes. Section 6 concludes.
...
\paragraph{Predictor-weight and predictor-set symmetry sensitivity:} Next we document two additional implementation checks for robustness. First, we vary the SCM predictor-weight optimization settings (initialization and method) while holding donor pools and timing fixed. Second, we re-estimate both country models under a harmonized reduced predictor set that uses common broad sectors (agriculture, manufacturing, construction, trade/hospitality, transport/information, finance/business, personal services, and public administration), while retaining country-specific auxiliary controls (GDP per capita and education). These diagnostics are reported in Section~4.3 and confirm that the qualitative treatment conclusions are not driven by one specific $v_m$ tuning choice or by unavoidable data asymmetries such as the Chile-specific Fishing category.
...
Figure~\ref{fig:maps} shows the geographic location of the two treated regions within their respective countries.
...
Now, to formally assess the significance of our findings, Figure \ref{fig:placebos} displays the results of the placebo tests. For Maule, its gap is neither noticeable nor statistically significant compared to the distribution of placebo gaps (Figure~\ref{fig:placebos}a). For Canterbury, in contrast, its gap stands out clearly as the highest in the post-2011 period, suggesting the effect is statistically significant (Figure~\ref{fig:placebos}b). Level vs. growth: it is relevant to mention that we interpret the post‑2011 divergence in Canterbury primarily as a one‑off \emph{level} increase in GDP per capita that is transitory and fades as the rebuild boost winds down and the insurance payoffs end up being absorbed throughout the economy (see also \cite{nguyen2020insurance}), rather than a permanent change in the growth rate, consistent with the gap decline to below 5\% by 2019 (see Figure~\ref{fig:gaps}b).
...
Figure~\ref{fig:uniform_sets} complements the placebo-spaghetti evidence with 95\% uniform confidence sets over post-treatment years. For Maule, the entire post-period band overlaps zero, consistent with the null effect. For Canterbury, the uniform band excludes zero only during part of the rebuild phase (roughly one quarter of post-treatment years), then widens back to include zero as the gap attenuates toward the end of the sample. This pattern supports a \emph{medium-lived} positive level effect rather than a permanent shift.
...
Quantitatively, the pre/post-RMSPE ratio for Maule is 0.48, and the associated pseudo-$p$ value is 1.00, confirming that it is not significant in rank terms. For Canterbury, the RMSPE ratio is 7.81, the highest among the 15 NZ regions in our donor pool, giving a one-sided pseudo-$p$ value of $1/15 \approx 0.067$ ($\approx$6.7\%). Together with the uniform sets, this implies that Canterbury’s positive effect is strongest in mid-rebuild years and less precisely estimated late in the horizon. Finally, it is relevant to note that our results are a measure of \emph{flows} (real GDP per capita); hence, it does not value the \emph{stock} losses from capital destruction or non-market welfare costs (mortality, morbidity, displacement, etc.). A positive post-quake GDP gap can therefore coexist with lower net wealth or welfare; our estimates therefore should be read as output responses, rather than a comprehensive welfare assessment \parencite{hallegatte2010economics}.

\subsection{Validation using night-time lights}
To address concerns about potential measurement noise in regional GDP accounts during reconstruction, we build a remote-sensing validation using annual night-time lights (NTL). Specifically, we aggregate a harmonized DMSP-OLS/VIIRS product to the same regional units used in the SCM and re-estimate parallel SCM specifications for Maule and Canterbury over 2000--2019, preserving treatment timing (2010 for Maule, 2011 for Canterbury) and donor-pool logic.

Figure~\ref{fig:ntl_validation} reports the resulting treated-vs-synthetic NTL paths and gaps. For Canterbury, the NTL evidence aligns with the GDP inference: the post-2011 gap is positive, with a 2016 gap of about 10.0\% and an average post-treatment gap of about 7.8\%. For Maule, however, NTL does \emph{not} replicate the GDP-null result: the NTL gap is positive (around 14.2\% in 2016; average post-treatment gap around 13.0\%). Hence, the night-time lights evidence confirms the direction of the Canterbury overshoot but diverges from the Maule GDP estimate.
...
Detailed timing, jackknife, and uniform-threshold sensitivity outputs are reported in Appendix~\ref{sec:additional_robustness_appendix}; the main qualitative conclusions are unchanged across all variants.
...
\subsection{Empirically anchored mechanism checks (suggestive, not definitive)}

To reduce reliance on narrative interpretation, we evaluate several reduced-form mechanism implications that are testable in our data and compare them with observed patterns. These are \emph{mechanism-consistent diagnostics}, not formal mediation estimates.

\paragraph{Liquidity/reconstruction channel prediction.}
If rapid reconstruction liquidity is central, we should observe: (i) a positive gap concentrated in rebuild years, (ii) stronger construction divergence than non-construction divergence, and (iii) attenuation as the rebuild pipeline winds down. This is the pattern in Canterbury: positive post-2011 gaps, statistically strong construction-share effects, and convergence toward synthetic by the end of the sample (Sections~4.1--4.3).

\paragraph{Pure denominator channel prediction.}
If GDP-per-capita gains were mainly demographic composition effects, total GDP should not co-move with per-capita gaps. In Canterbury, decomposition evidence indicates a positive total-GDP gap alongside a negative population gap, implying denominator effects contribute but do not fully account for the overshoot. For Maule, available decomposition evidence does not reveal a hidden positive output overshoot.
...
Taken jointly, these diagnostics are consistent with a mechanism bundle in which insurance-linked liquidity, implementation capacity, and temporary sectoral reallocation help explain Canterbury's medium-lived overshoot, whereas Maule maps into return-to-trend dynamics. We therefore frame mechanisms as \emph{suggestive and empirically disciplined}, not as fully identified causal channels.



\subsection{Limitations and policy relevance}
We emphasize that these mechanisms and the evidence gathered in this paper suggest strong correlations consistent with the timing and sectoral facts we document; however, our SCM design does not \emph{causally} identify the marginal effect of any one specific institutional feature. Nor does GDP capture welfare losses. Put differently, a caveat is worth emphasizing: a limitation of our analysis is that, due to the lack of empirical micro-data and due to the simultaneous channels involved in driving the effect of natural disasters, we cannot isolate and identify the impact of each individual channel aforementioned in the subsection on institutions as mechanisms.
