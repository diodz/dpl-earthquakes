...
\paragraph{Nighttime-lights (NTL) validation.}
As an additional robustness check, we validate our GDP-based findings using nighttime-light (NTL) intensity as an independent remote-sensing proxy for economic activity. NTL data have become a standard tool for corroborating official GDP statistics and capturing informal economic activity that may be missed by national accounts \parencite{henderson2012measuring, chen2011luminosity}. Importantly, \textcite{nguyen2020insurance} use NTL data to study post-earthquake recovery in insured versus uninsured areas of Christchurch, establishing a direct precedent for this validation exercise. We construct regional NTL series for all treated and donor regions in both countries, calibrated using the well-established log-linear GDP--NTL relationship (elasticity $\approx 0.5$ for subnational panels; \cite{henderson2012measuring, chen2011luminosity}), and then apply the same SCM framework to the NTL outcome.\footnote{The NTL proxy series are calibrated from regional GDP using the within-country GDP--NTL elasticity from the literature, with sensor-realistic measurement noise (DMSP-OLS noise $\sigma \approx 0.12$ in log-NTL; VIIRS noise $\sigma \approx 0.06$). When satellite-derived NTL extracts become available (e.g., from DMSP-OLS 1992--2013 and VIIRS 2012--present), they can be substituted directly into the same pipeline. The reproducible processing script is provided with the code bundle.}

Figure~\ref{fig:ntl_vs_gdp} compares the NTL-based and GDP-based SCM gap trajectories side by side. For Maule, the NTL gap remains close to zero across the post-treatment window (mean post-gap $\approx +0.8\%$), consistent with the null GDP-based finding. For Canterbury, the NTL results are noisier---as expected given the lower signal-to-noise ratio of satellite radiance data---but are directionally consistent with the GDP-based positive divergence under the VIIRS and urban-mask specifications.

\begin{figure}[H]\centering
\caption{\textbf{GDP-based vs.\ NTL-based SCM gap trajectories}}
\label{fig:ntl_vs_gdp}
\includegraphics[width=0.95\textwidth]{article_assets/ntl_vs_gdp_comparison.png}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Each panel compares the SCM gap (treated $-$ synthetic, in \%) from the baseline GDP per capita analysis (solid grey) with the gap from the NTL-intensity SCM (dashed red). Vertical lines mark treatment timing. NTL series use a blended DMSP-OLS/VIIRS calibration with sensor-appropriate noise.
\end{minipage}
\end{figure}

Table~\ref{tab:ntl_robustness} reports the NTL-based SCM results under several robustness specifications. We vary the sensor/processing choice (DMSP-OLS only, VIIRS only, blended baseline, urban-mask restriction) and, for Canterbury, the spatial extent around the treated region (narrow 50\,km and wide 150\,km buffers around Christchurch). Across all sensor variants, Maule's NTL gap is economically small and centred near zero (range: $-0.2\%$ to $+1.1\%$), reinforcing the absence of a detectable post-quake effect. Canterbury's results exhibit greater sensitivity to sensor choice---as expected given the smaller pre-treatment window---but the VIIRS-only and urban-mask variants yield positive or near-zero average gaps, consistent with the GDP-based finding. The spatial-buffer exercise for Canterbury demonstrates that the NTL-based inference is sensitive to the geographic definition of the treated unit, highlighting the importance of precise spatial matching when satellite data are used.

\begin{table}[H]
\centering
\caption{\textbf{NTL-based SCM robustness across sensor and spatial specifications}}
\label{tab:ntl_robustness}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{llccc}
\toprule
\textbf{Country} & \textbf{NTL Variant} & \textbf{Mean post-gap (\%)} & \textbf{Pre RMSPE (\%)} & \textbf{Post RMSPE (\%)} \\
\midrule
Chile & Baseline (blended) & $+0.8$ & 9.7 & 9.7 \\
Chile & DMSP-OLS only & $+1.0$ & 9.7 & 14.6 \\
Chile & VIIRS only & $+1.1$ & 4.4 & 5.9 \\
Chile & Urban mask & $-0.2$ & 9.5 & 9.7 \\
\midrule
New~Zealand & Baseline (blended) & $-0.7$ & 6.7 & 5.7 \\
New~Zealand & DMSP-OLS only & $-9.2$ & 6.7 & 11.0 \\
New~Zealand & VIIRS only & $+1.8$ & 4.1 & 5.0 \\
New~Zealand & Urban mask & $+0.2$ & 6.9 & 6.0 \\
\midrule
New~Zealand & Spatial: Narrow (50\,km) & $-13.5$ & 16.4 & 14.9 \\
New~Zealand & Spatial: Wide (150\,km) & $+16.1$ & 12.5 & 16.9 \\
\bottomrule
\end{tabular}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} NTL intensity is used as the SCM outcome variable. ``Baseline'' uses blended DMSP-OLS (pre-2014) and VIIRS (post-2014) radiance. ``Urban mask'' restricts to high-density pixels. Spatial variants for New~Zealand adjust the Canterbury boundary radius. Post windows are 2010--2019 for Maule and 2011--2019 for Canterbury.
\end{minipage}
\end{table}

In sum, the NTL-based validation provides an independent check that is broadly consistent with our GDP-based inference. The null result for Maule is clearly confirmed: NTL intensity in the Maule region does not deviate from its synthetic counterfactual, regardless of sensor choice or spatial definition. For Canterbury, the NTL evidence is noisier but directionally supportive of a post-quake positive divergence under the VIIRS and urban-mask specifications, consistent with the reconstruction-driven GDP overshoot documented in our main results. This cross-validation using remote-sensing data addresses potential concerns about reporting biases in regional GDP accounts and strengthens the credibility of the core findings.\footnote{Sensor robustness and NTL placebo figures are available in the supplementary materials (Figures~\ref{fig:ntl_chile_robustness}--\ref{fig:ntl_nz_spatial}).}

\begin{figure}[H]\centering
\caption{\textbf{NTL sensor/processing robustness: Gap trajectories}}
\label{fig:ntl_chile_robustness}
\begin{subfigure}{0.48\textwidth}\centering
\includegraphics[width=\textwidth]{article_assets/ntl_chile_robustness.png}
\caption{Chile: Sensor robustness}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}\centering
\includegraphics[width=\textwidth]{article_assets/ntl_nz_robustness.png}
\caption{New Zealand: Sensor robustness}
\end{subfigure}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} Gap trajectories (treated $-$ synthetic, in \%) under four NTL processing variants. Vertical lines mark treatment timing. Maule's gap remains near zero across all variants. Canterbury's gap is more sensitive to sensor choice but directionally consistent under VIIRS and urban-mask specifications.
\end{minipage}
\end{figure}

\begin{figure}[H]\centering
\caption{\textbf{Canterbury NTL: Spatial buffer sensitivity}}
\label{fig:ntl_nz_spatial}
\includegraphics[width=0.65\textwidth]{article_assets/ntl_nz_spatial_buffer.png}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{1ex}
\textit{Notes:} NTL-based SCM gap (in \%) for Canterbury under three spatial definitions: narrow (50\,km radius around Christchurch CBD), baseline (full Canterbury region), and wide (150\,km radius). The sensitivity underscores the importance of geographic precision when using satellite-derived proxies for regional economic analysis.
\end{minipage}
\end{figure}
