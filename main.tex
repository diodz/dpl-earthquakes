...
In sum, the NTL-based analysis demonstrates that the SCM framework produces consistent results when applied to a calibrated proxy outcome. The null result for Maule is clearly confirmed: NTL intensity in the Maule region does not deviate from its synthetic counterfactual, regardless of sensor choice or spatial definition. For Canterbury, the NTL evidence is noisier but directionally supportive of a post-quake positive divergence under the VIIRS and urban-mask specifications, consistent with the reconstruction-driven GDP overshoot documented in our main results. When actual satellite-derived NTL data are substituted for the calibrated proxy series, this approach would provide an independent check on reporting biases in regional GDP accounts.\footnote{Sensor robustness and NTL placebo figures are available in the supplementary materials (Figures~\ref{fig:ntl_chile_robustness}--\ref{fig:ntl_nz_spatial}).}
