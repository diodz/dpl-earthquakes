...
We also implement spatial damage controls in measurement. First, we re-estimate night-time-lights validation paths using urban-core masks (Talca-Curicó-Linares-Cauquenes in Maule; Christchurch-centered core in Canterbury) rather than only full administrative regions. Second, for Canterbury we add concentric 30/50/80 km buffers centered on Christchurch and show that all buffers share the same qualitative post-2011 pattern (initial dip, sustained rebuild rise). Together with the progressive geographic donor exclusions in Appendix~\ref{sec:spillover_appendix}, these checks reduce the risk that our cross-case contrast is driven solely by within-region spatial composition differences.

Timing-sensitivity outputs are available in the replication package.
...
Table~\ref{tab:spillover_exclusion_summary} reports the results. Figure~\ref{fig:spillover_gap_paths} overlays the gap paths under each exclusion ring.
...
Spillover-diagnostics outputs are available in the replication package.
...
\include{response_to_reviewers.tex}
\end{document}
